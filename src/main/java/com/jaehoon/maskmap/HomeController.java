...
public class HomeController {
    ...
    // @RequestMapping(value = "/dataInsert") 데이터 업로드. mysql서버로 쓰고 있는 ec2에서 매일 새벽4시에
    // curl 요청을 보내 업로드를 한다.
    // @ResponseBody
    // public void dataInsert() {
    // for (int z = 1; z < 7; z++) {
    // String url =
    // "https://8oi9s0nnth.apigw.ntruss.com/corona19-masks/v1/sales/json?page=" + z
    // + "&perPage=5000";
    // RestTemplate restTemplate = new RestTemplate();
    // HttpHeaders headers = new HttpHeaders();
    // headers.setContentType(new MediaType("application", "json",
    // Charset.forName("UTF-8"))); // Response Header
    // // to
    // // UTF-8
    ...
    // if (obj.isNull("stock_at")) 입고 기록이 없는 판매처는 제외
    // continue Loop1;
    ...
    // for (StoreInfo data : list) { 어제 입고한 이후로 입고가 되지않아 같은 데이터가 또 들어올 수 있으므로 해당
    ...
    // if (list.size() == 7) { 총 7일간의 데이터만을 수집하므로 이미 7일간의 데이터가 있다면 가장 오래된 데이터를 삭제한 후
    // 새 데이터를 넣는다.
    // Collections.sort(list);

    // storeInfoRepository.deleteByStockTime(list.get(0).getStockTime()); 입고시간을 기준으로
    // 오름차순 정렬해준 후 가장 오래된 데이터를 삭제.

    // storeinfo.setCode(code);
    // storeinfo.setStockTime(stock_at2);
    // storeInfoRepository.save(storeinfo);

    // continue Loop1; 삭제 후 새 데이터를 넣고 다음 루프로.

    // }

    // storeinfo.setCode(code); 데이터가 7개가 아니고 중복도 없을 시에 저장.
    // storeinfo.setStockTime(stock_at2);
    // storeInfoRepository.save(storeinfo);

    // double cal = 0;
    // List<StoreInfo> list2 = new ArrayList<>();
    // list2 = storeInfoRepository.findByCode(code); 새로 들어온 데이터를 다시 가져와 리스트에 넣는다.

    // for (StoreInfo data : list2) {

    // String a = data.getStockTime(); 입고 시간에서 시, 분을 분리한다.
    // String b = a.substring(4, 6);
    // String c = a.substring(6, 8);

    // cal = cal + (Integer.parseInt(b) * 60 + Integer.parseInt(c)); 시간에 60을 곱한 후 분과
    // 더해주어 단위를 맞춰준 후 반복문을 통해 더해준다.

    // }
    // double result = cal / list2.size() / 60; n개의 입고시간데이터를 데이터 개수만큼 나눠준 뒤 60을 곱해 시
    // 단위로 변환해준다.

    // String str = Double.toString(Math.round(result * 100) / 100.0); 변환된 시간의 소수점을
    // 두자리까지 반올림해준다.

    // String[] hour = str.split("\\."); // .과 같은 예약어를 구분자로 쓸때는 \\를 붙여준다.
    // double min = 0; 시간을 시 분으로 나누어 배열에 저장한다.
    // if (hour[0].length() == 1) { 10시이전 즉 1~9에 시간에 한 해서 앞에 0을 붙여준다.
    // hour[0] = "0" + hour[0];
    // }
    // if (hour[1].length() == 1) { 분은 소수점이 1자리인 값에 대해서만 0을 붙여준다.
    // 소수점은 분단위로 변환시켜야 하기때문에 미리 0을 붙여서 값을 맞춰준다.
    // hour[1] = hour[1] + "0";
    // }
    // min = Double.parseDouble(hour[1]) * 0.6; 소수점에 0.6을 곱하여 분단위로 변환해준다. 위에서 0을
    // 붙여줘도 되고 여기서 6을 곱해줘도 값은 같다.
    // min = Math.round(min); 반올림해준다.
    // int min2 = (int) min;
    // String minStr = "";
    // if (min2 < 10) {
    // minStr = "0" + Integer.toString(min2); 10보다 작은 값에 대해 0을 붙여준다.
    // } else {
    // minStr = Integer.toString(min2);
    // }
    // String finalResult = hour[0] + ":" + minStr; 평균 시간이 구해졌다.

    // AvgTime avgTime = new AvgTime();

    // avgTime.setCode(code);
    // avgTime.setAvgTime(finalResult);
    // avgTime.setDay(list2.size()); 평균시간과 리스트의 개수, 즉 데이터가 수집된 일 수를 저장한다.
